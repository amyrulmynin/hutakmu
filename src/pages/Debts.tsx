...
export default function Debts() {
  const debts = [
    { id: "1", borrower: "Ahmad", amount: 650, monthly: 54.17, months: 12, paid: 3, status: "active" },
    { id: "2", borrower: "Siti", amount: 350, monthly: 58.33, months: 6, paid: 6, status: "completed" },
    { id: "3", borrower: "Ali", amount: 1100, monthly: 91.67, months: 12, paid: 1, status: "active" },
  ]
          ...
          <Card key={debt.id}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-sm">{debt.borrower}</p>
                  <p className="text-xs text-muted-foreground">RM{debt.monthly}/bulan x {debt.months} bulan</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-sm">RM {debt.amount}</p>
                  <span className={`text-[10px] px-2 py-0.5 rounded ${debt.status === "active" ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"}`}>
                    {debt.status === "active" ? `${debt.paid}/${debt.months} paid` : "Selesai"}
                  </span>
                </div>
              </div>
